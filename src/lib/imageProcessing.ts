...
export interface OutlineOptions {
  ...
  customParams?: {
    ...
    min_area?: number;
    // ImageTracerJS quality parameters
    ltres?: number; // Line fitting error threshold (lower = more precise)
    qtres?: number; // Quadratic spline fitting error threshold (lower = more precise)
    pathomit?: number; // Minimum path length (lower = keep more detail)
    rightangleenhance?: boolean; // Enhance right angles (false for smoother curves)
    roundcoords?: number; // Decimal places for coordinates (higher = more precision)
    linefilter?: boolean; // Apply line filter for cleaner output
    colorsampling?: number; // Color sampling method (0=disabled, 1=random, 2=deterministic)
    numberofcolors?: number; // Maximum number of colors
    blurradius?: number; // Blur radius for preprocessing
...
export async function createOutlinePortraitOptimized(
  ...
  try {
    ...
    let result = sharp(preprocessed).convolve({
      width: 3,
      height: 3,
      kernel: Array(9)
        .fill(-1)
        .map((_, i) => (i === 4 ? edgeStrength : -1)),
    });
...
export async function createOutlineHighContrast(
  ...
  try {
    ...
    let result = sharp(imageBuffer).grayscale().gamma(gamma).normalize();
...
export async function createOutlineArtistic(
  ...
  try {
    ...
    let result = sharp(posterized).convolve({
      width: 3,
      height: 3,
      kernel: [-1, -1, -1, -1, 8, -1, -1, -1, -1],
    });
...
export async function createOutlineAIEdgeDetection(
  ...
  try {
    ...
    const high_threshold =
      customParams?.high_threshold ?? params.high_threshold;
    const blur_kernel = customParams?.blur_kernel ?? params.blur_kernel;

    const backendUrl =
      process.env.PYTHON_BACKEND_URL || "http://localhost:9000";
    ...
    const queryParams = new URLSearchParams({
      ...
      invert_colors:
        customParams?.invert_colors ?? invertColors ? "true" : "false",
    });
...
export async function createOutlineColoringXDoG(
  ...
  try {
    ...
    const bilateral_sigma_color =
      customParams?.bilateral_sigma_color ?? params.bilateral_sigma_color;
    const bilateral_sigma_space =
      customParams?.bilateral_sigma_space ?? params.bilateral_sigma_space;
    ...
    const line_thickness =
      customParams?.line_thickness ?? params.line_thickness;
    ...
    formData.append(
      "invert_colors",
      (customParams?.invert_colors ?? invertColors).toString()
    );
...
export async function createOutlineSVGVectorTrace(
  ...
  try {
    ...
    const settings = {
      light: {
        threshold: 180,
        turdSize: 8, // Standard detail
        ltres: 1, // Standard line precision
        qtres: 1, // Standard curve precision
        roundcoords: 1, // 1 decimal place
        linefilter: false, // No filtering for speed
      },
      medium: {
        threshold: 128,
        turdSize: 2, // High detail
        ltres: 0.5, // Higher line precision
        qtres: 0.5, // Higher curve precision
        roundcoords: 2, // 2 decimal places
        linefilter: true, // Clean up paths
      },
      strong: {
        threshold: 100,
        turdSize: 0, // Maximum detail (keep all paths)
        ltres: 0.1, // Maximum line precision
        qtres: 0.1, // Maximum curve precision
        roundcoords: 3, // 3 decimal places
        linefilter: true, // Clean up paths
      },
    };
    ...
    const ltres = customParams?.ltres ?? params.ltres;
    const qtres = customParams?.qtres ?? params.qtres;
    const roundcoords = customParams?.roundcoords ?? params.roundcoords;
    const linefilter = customParams?.linefilter ?? params.linefilter;

    // Create a high-contrast B/W image suitable for tracing
    let processed = sharp(imageBuffer).grayscale().threshold(threshold);
    ...
    // High-quality ImageTracer options with user customization
    const tracerOptions = {
      // Precision settings - key for quality!
      ltres: ltres, // Line fitting error threshold
      qtres: qtres, // Quadratic spline fitting error threshold
      pathomit: turdSize, // Minimum path length to keep
      rightangleenhance: customParams?.rightangleenhance ?? false, // Smooth curves
      roundcoords: roundcoords, // Coordinate precision

      // Path enhancement
      linefilter: linefilter, // Clean up paths

      // Color settings
      colorsampling: customParams?.colorsampling ?? 1, // Random sampling for quality
      numberofcolors: customParams?.numberofcolors ?? 2, // Black and white
      mincolorratio: 0.02, // Minimum color ratio
      colorquantcycles: 3, // Color quantization iterations

      // Preprocessing
      blurradius: customParams?.blurradius ?? 0, // Preserve sharp details
      blurdelta: 20, // Blur delta

      // Output settings
      scale: customParams?.scale ?? 1, // No scaling
      strokewidth: 1, // Thin strokes
      desc: false, // No description
      viewbox: false, // No viewbox

      // Advanced quality settings
      corsenabled: false, // Disable corner enhancement
      layering: 0, // Single layer
      lcpr: 0, // Line control point ratio
      qcpr: 0, // Quadratic control point ratio
    };

    console.log(
      `High-quality SVG tracing [${intensity}] - threshold: ${threshold}, ltres: ${ltres}, qtres: ${qtres}, pathomit: ${turdSize}, precision: ${roundcoords}`
    );
    ...
    console.log(
      `High-quality ImageTracerJS completed: ${svgString.length} characters`
    );
